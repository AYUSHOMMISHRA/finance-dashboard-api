...
describe('Records Module', () => {
  ...
  describe('GET /records', () => {
    ...
    it('should filter by type=INCOME', async () => {
      ...
      expect(res.status).toBe(200);
      res.body.data.forEach((record: { type: string }) => {
    ...
    it('should search in notes and category', async () => {
      ...
      const analystToken = await getAnalystToken();
      const res = await request(app)
    ...
    it('should filter by date range', async () => {
      ...
      const res = await request(app)
        .get(
          `/api/v1/records?startDate=${startDate.toISOString().split('T')[0]}&endDate=${endDate.toISOString().split('T')[0]}`
        )
        .set('Authorization', `Bearer ${token}`);
  ...
  describe('PATCH /records/:id', () => {
    it('should update record as ADMIN', async () => {
      const token = await getAdminToken();
      const record = await createTestRecord(token, { category: 'patch-test' });
      createdRecordIds.push(record.id);
    ...
    it('should clear notes with null', async () => {
      const token = await getAdminToken();
      const record = await createTestRecord(token, { notes: 'Will be cleared' });
      createdRecordIds.push(record.id);
    ...
    it('should return 404 for soft-deleted record', async () => {
      const token = await getAdminToken();
      const record = await createTestRecord(token);
      createdRecordIds.push(record.id);
  ...
  describe('DELETE /records/:id', () => {
    ...
    it('should exclude deleted records from list', async () => {
      const token = await getAdminToken();
      const record = await createTestRecord(token, { category: 'delete-test' });
      createdRecordIds.push(record.id);
